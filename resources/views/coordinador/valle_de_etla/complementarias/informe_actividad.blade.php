...
<title>Informe de Actividades Complementarias - ITVE</title>
<style>
.boton-actividad {
  padding: 8px 16px;
...
<div class="titulo-principal">
  <h1>Informe de Actividades Complementarias</h1>
...
<div class="seccion-contenedor">
  ...
  <div class="seccion activa" id="seccion1">
    ...
    <input type="hidden" id="id_unidad" value="{{ $id_unidad ?? 4 }}">
    <input type="hidden" id="tipo_programa_informe" value="{{ $tipo_programa_informes_panel ?? \App\Models\Actividad::TIPO_COMPLEMENTARIA }}">
        ...
        @foreach($informes as $inf)
          <div class="documento-card tarjeta-documento" style="background: #fff; border-radius: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.07); padding: 16px; min-width: 260px; max-width: 300px; display: flex; flex-direction: column; align-items: flex-start; border: 1px solid #e1e5eb;">
            ...
            <div class="documento-actions" style="margin-top: 10px; display: flex; gap: 8px;">
              ...
              <form method="POST" action="{{ route('coordinador.valle_etla.informe.eliminar', $inf->id) }}" onsubmit="return confirmarEliminacionInforme(event)">
                @csrf
                @method('DELETE')
                <input type="hidden" name="tipo_programa" value="{{ $tipo_programa_informes_panel ?? \App\Models\Actividad::TIPO_COMPLEMENTARIA }}">
  ...
  <div class="seccion" id="seccion4">
    ...
    <div style="background: #f8f9fa; padding: 20px; border-radius: 8px; margin: 24px 0 20px 0; border-left: 4px solid #17a2b8;">
      ...
      <div class="informe-firmas-grid" style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px;">
        ...
        <div style="background: #fff; padding: 16px; border-radius: 8px; border: 1px solid #dee2e6; box-shadow: 0 1px 4px rgba(0,0,0,0.04);">
          ...
          <p id="firmaCargoJefeDepartamento" class="informe-firma-cargo" data-default="Jefe de Departamento Actividades Complementarias" contenteditable="false" title="Doble clic para editar el cargo">Jefe de Departamento Actividades Complementarias</p>
        </div>
      </div>
    </div>
    <style>
    ...
    <div class="grupo-formulario informe-lugar-fecha-grupo">
      ...
      <input type="text" id="lugarFecha" class="input-estilo" placeholder="Ej: Santiago Suchilquitongo, a los 27 días del mes de mayo de 2025">
...
@php
  $tipoProgramaInforme = $tipo_programa_informe ?? \App\Models\Actividad::TIPO_COMPLEMENTARIA;
  $actividadesReporte = \App\Models\Actividad::where('id_semestre', $id_semestre ?? ($semestreActual->id_semestre ?? null))
      ->where('id_unidad', $id_unidad ?? 4)
...
function inicializarTituloInformeAuto() {
  ...
  function autoTitulo() {
    ...
    return 'Informe de Actividades Complementarias (' + p + ')';
  }
  function sync() {
  ...
  switch(seccionActual) {
    ...
    case 2: // Información del informe
      ...
      if (actividadCultural !== 'X' && actividadDeportiva !== 'X') {
        swalAlerta('Debe seleccionar al menos una actividad (Cultural o Deportiva).');
        return false;
      }
      return true;
...
async function generarPDF() {
  ...
  if (!firmasNombresCoordinadorRequeridosOk()) {
    ocultarCargaInforme();
    swalAlerta('Debe escribir el nombre completo del promotor cultural y del jefe de oficina en las firmas del informe.');
    return;
  }
  ...
  try {
        ...
        didDrawPage: function (data) {
          if (index === bloques.length - 1) {
            ...
            const bloquesFirma = [
              ...
              { id: 'firmaNombreJefeDepartamento', cargoId: 'firmaCargoJefeDepartamento', cargoDefault: 'Jefe de Departamento Actividades Complementarias' }
            ];
    ...
    fetch(@json(route('informe_valle_de_etla.guardar')), {
      method: 'POST',
      headers,
