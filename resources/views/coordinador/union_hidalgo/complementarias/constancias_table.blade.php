...
<script>
    ...
    @endphp
    @php
        $semIdForStorage = $semestre->id_semestre ?? $semestre->id ?? (\App\Models\Semestre::where('estatus',1)->first()->id_semestre ?? '');
    @endphp
    
    const todosEstudiantes = @json($estudiantesArray);
    const semIdForStorage = '{{ $semIdForStorage }}';
    ...
    function renderizarPagina(pagina) {
        ...
        if (estudiantesEnPagina.length === 0) {
            ...
            estudiantesEnPagina.forEach(s => {
                ...
                // botón de acción: Evaluar o Descargar según estado
                const savedDoc = localStorage.getItem(`membreteDocId_${semIdForStorage}`) || '';
                let actionHtml = '';
                if (s.status === 'completed' && s.id_evaluacion) {
                    // estilo y estructura iguales a Tlahuitoltepec (icono + color)
                    actionHtml = `
    ...
    // Determinar semestre activo para fechas y persistencia
    const semestreActivo = {
        fechaInicio: '{{ $semestre->fecha_inicio ?? "" }}',
        fechaFin: '{{ $semestre->fecha_fin ?? "" }}',
        periodo: '{{ $semestre->nombre ?? "" }}'
    };

    // Persistencia del documento membretado seleccionado (por semestre) usando localStorage
    (function persistenciaDocumentoMembrete() {
        ...
        const storageKey = `membreteDocId_${semIdForStorage || 'global'}`;

        const saved = localStorage.getItem(storageKey);
        if (saved && select) {
            select.value = saved;
            if (select.value) estado.style.display = '';
        }
    ...
    function abrirModalEvaluacion(estudiante) {
        ...
        modal.style.cssText = `position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.6); display: flex; justify-content: center; align-items: center; z-index: 10000; overflow-y: auto; padding: 20px;`;
        ...
        modal.innerHTML = `
            <div style="background: white; border-radius: 12px; max-width: 1000px; width: 100%; max-height: 90vh; overflow-y: auto; box-shadow: 0 10px 40px rgba(0,0,0,0.3);">
                <div style="background: linear-gradient(135deg, #1a3461 0%, #2d5aa0 100%); color: white; padding: 25px 30px; border-radius: 12px 12px 0 0; position: relative;">
                    <h2 style="margin: 0 0 8px 0; font-size: 1.8em; font-weight: 700;"><i class="fas fa-clipboard-check" style="margin-right: 10px;"></i>Evaluación de Desempeño</h2>
                    <p style="margin: 0; opacity: 0.9; font-size: 0.95em;">Actividad complementaria</p>
                    <button onclick="cerrarModalEvaluacion()" style="position: absolute; top: 20px; right: 20px; background: rgba(255,255,255,0.2); border: none; color: white; width: 36px; height: 36px; border-radius: 50%; cursor: pointer; font-size: 1.3em;"><i class="fas fa-times"></i></button>
                </div>
                <div style="padding: 30px;">
                    <form id="formEvaluacion">
                        <div style="background: #f8f9fa; padding: 20px; border-radius: 8px; margin-bottom: 25px; border-left: 4px solid #1a3461;">
                            <h3 style="margin: 0 0 15px 0; color: #1a3461; font-size: 1.2em; display: flex; align-items: center; gap: 8px;"><i class="fas fa-user-graduate"></i> Información del Estudiante</h3>
                            <div style="display: grid; grid-template-columns: 1fr; gap: 15px;">
                                <div>
                                    <label style="display: block; font-weight: 600; color: #333; margin-bottom: 6px; font-size: 0.95em;">(1) Nombre del estudiante:</label>
                                ...
                                <div>
                                    <label style="display: block; font-weight: 600; color: #333; margin-bottom: 6px; font-size: 0.95em;">(2) Actividad complementaria:</label>
                                ...
                                <div>
                                    <label style="display: block; font-weight: 600; color: #333; margin-bottom: 6px; font-size: 0.95em;">(3) Periodo de realización:</label>
                                    <div style="margin-bottom: 10px; padding: 8px 12px; background: #e7f3ff; border-radius: 6px; border-left: 3px solid #0d6efd;">
                                        <small style="color: #0d6efd; font-weight: 600;"><i class="fas fa-calendar-alt"></i> Semestre: <span id="periodoSemestre"></span></small>
                                    </div>
                        ...
                        <div style="margin-bottom: 25px;">
                            <h3 style="margin: 0 0 15px 0; color: #1a3461; font-size: 1.2em; display: flex; align-items: center; gap: 8px;"><i class="fas fa-tasks"></i> (4) Nivel de desempeño del criterio</h3>
                            <p style="color: #6c757d; font-size: 0.9em; margin: 0 0 20px 0; padding: 12px; background: #fff3cd; border-left: 4px solid #ffc107; border-radius: 6px;"><i class="fas fa-info-circle"></i> Seleccione el nivel de desempeño alcanzado por el estudiante en cada criterio</p>
                            <div style="overflow-x: auto;"><table id="tablaCriterios" style="width: 100%; border-collapse: collapse; border: 1px solid #dee2e6; table-layout: auto;"><thead><tr style="background: #1a3461; color: white;"><th style="padding: 12px; text-align: center; border: 1px solid #dee2e6; width: 50px; vertical-align: middle;">No.</th><th style="padding: 12px; text-align: left; border: 1px solid #dee2e6; min-width: 300px; vertical-align: middle;">Criterios a evaluar</th><th style="padding: 12px; text-align: center; border: 1px solid #dee2e6; width: 90px; vertical-align: middle;">Insuficiente<br><small>(0)</small></th><th style="padding: 12px; text-align: center; border: 1px solid #dee2e6; width: 90px; vertical-align: middle;">Suficiente<br><small>(1)</small></th><th style="padding: 12px; text-align: center; border: 1px solid #dee2e6; width: 90px; vertical-align: middle;">Bueno<br><small>(2)</small></th><th style="padding: 12px; text-align: center; border: 1px solid #dee2e6; width: 90px; vertical-align: middle;">Notable<br><small>(3)</small></th><th style="padding: 12px; text-align: center; border: 1px solid #dee2e6; width: 90px; vertical-align: middle;">Excelente<br><small>(4)</small></th></tr></thead><tbody>${generarFilasCriterios()}</tbody></table></div>
                        </div>
                        ...
                        <div style="background: #e7f3ff; padding: 20px; border-radius: 8px; border-left: 4px solid #0d6efd;">
                            <h3 style="margin: 0 0 15px 0; color: #0d6efd; font-size: 1.2em; display: flex; align-items: center; gap: 8px;"><i class="fas fa-calculator"></i> Resultados de la Evaluación</h3>
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;"><div><label style="display: block; font-weight: 600; color: #333; margin-bottom: 6px; font-size: 0.95em;">(7) Valor numérico de la actividad:</label><input type="text" id="valorNumerico" readonly value="0.00" style="width: 100%; padding: 10px 12px; border: 1px solid #0d6efd; border-radius: 6px; background: white; color: #0d6efd; font-weight: 700; font-size: 1.1em; text-align: center;"></div><div><label style="display: block; font-weight: 600; color: #333; margin-bottom: 6px; font-size: 0.95em;">(8) Nivel de desempeño alcanzado:</label><input type="text" id="nivelDesempeno" readonly value="No evaluado" style="width: 100%; padding: 10px 12px; border: 1px solid #0d6efd; border-radius: 6px; background: white; color: #0d6efd; font-weight: 700; font-size: 1.1em; text-align: center;"></div></div>

                        <div style="display: flex; gap: 12px; justify-content: flex-end; margin-top: 30px; padding-top: 20px; border-top: 2px solid #e9ecef;">
                            <button type="button" onclick="cerrarModalEvaluacion()" style="padding: 12px 28px; border: 2px solid #6c757d; background: white; color: #6c757d; border-radius: 6px; cursor: pointer; font-weight: 600; font-size: 0.95em;"> <i class="fas fa-times"></i> Cancelar</button>
                            <button type="submit" style="padding: 12px 28px; border: none; background: linear-gradient(135deg, #28a745 0%, #20c997 100%); color: white; border-radius: 6px; cursor: pointer; font-weight: 600; font-size: 0.95em; box-shadow: 0 4px 12px rgba(40,167,69,0.3);"> <i class="fas fa-save"></i> Guardar Evaluación</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    function cargarFechasSemestre() {
        ...
        if (semestreActivo.periodo) periodoElement.textContent = semestreActivo.periodo; else periodoElement.textContent = 'No definido';
        if (semestreActivo.fechaInicio) fechaInicioInput.value = semestreActivo.fechaInicio;
        if (semestreActivo.fechaFin) fechaTerminoInput.value = semestreActivo.fechaFin;
    }
        ...
        criterios.forEach((criterio, index) => {
            ...
            html += `
                <tr style="background: ${index % 2 === 0 ? '#ffffff' : '#f8f9fa'};">
                    ...
                    <td style="padding: 12px; text-align: center; border: 1px solid #dee2e6; vertical-align: middle;"><input type="radio" name="criterio${num}" value="0" class="criterio-radio" required style="width: 20px; height: 20px; cursor: pointer;"></td>
    ...
    function agregarCalculosAutomaticos() {
        ...
        radios.forEach(radio => radio.addEventListener('change', calcularResultados));
    }

    function calcularResultados() {
        let suma = 0; let criteriosEvaluados = 0;
        for (let i = 1; i <= 7; i++) {
            const radioSeleccionado = document.querySelector(`input[name="criterio${i}"]:checked`);
            if (radioSeleccionado) { suma += parseFloat(radioSeleccionado.value); criteriosEvaluados++; }
        }
        if (criteriosEvaluados === 7) {
            const promedio = suma / 7; document.getElementById('valorNumerico').value = promedio.toFixed(2);
            let nivelDesempeno = '';
            if (promedio >= 3.50 && promedio <= 4.00) { nivelDesempeno = 'Excelente'; document.getElementById('nivelDesempeno').style.color = '#28a745'; }
            else if (promedio >= 2.50 && promedio < 3.50) { nivelDesempeno = 'Notable'; document.getElementById('nivelDesempeno').style.color = '#17a2b8'; }
            else if (promedio >= 1.50 && promedio < 2.50) { nivelDesempeno = 'Bueno'; document.getElementById('nivelDesempeno').style.color = '#ffc107'; }
            else if (promedio >= 1.00 && promedio < 1.50) { nivelDesempeno = 'Suficiente'; document.getElementById('nivelDesempeno').style.color = '#fd7e14'; }
            else if (promedio >= 0.00 && promedio < 1.00) { nivelDesempeno = 'Insuficiente'; document.getElementById('nivelDesempeno').style.color = '#dc3545'; }
            document.getElementById('nivelDesempeno').value = nivelDesempeno;
        } else {
            document.getElementById('valorNumerico').value = '0.00'; document.getElementById('nivelDesempeno').value = 'No evaluado'; document.getElementById('nivelDesempeno').style.color = '#0d6efd';
        }
    }
    ...
    function guardarEvaluacion(estudiante) {
        ...
        if (!jefeExtraescolares) { swalAlerta('Por favor, ingrese el nombre del Subdirector Académico.'); document.getElementById('jefeExtraescolares').focus(); return; }
        ...
        fetch('{{ route("unionhidalgo.evaluacion.guardar") }}', {
            method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' }, body: JSON.stringify(datosEvaluacion)
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                cerrarModalEvaluacion();
                const urlPDF = `/coordinador/union-hidalgo/complementarias/constancia/${data.id_evaluacion}/pdf?id_documento=${documentoMembrete}`;
                ...
                setTimeout(() => { Swal.fire({ title: 'Evaluación guardada', text: 'Descargando constancia...', icon: 'success', draggable: true, showConfirmButton: false, timer: 1800, timerProgressBar: true }); }, 400);
            } else {
                Swal.fire({ title: 'Error al guardar', text: (data.message || 'Ocurrió un problema al guardar la evaluación'), icon: 'error', draggable: true, showConfirmButton: false, timer: 2200, timerProgressBar: true });
                btnGuardar.disabled = false; btnGuardar.innerHTML = textoOriginal;
            }
        })
        .catch(error => { console.error('Error:', error); Swal.fire({ title: 'Error de conexión', text: 'No se pudo conectar al servidor. Intente nuevamente.', icon: 'error', draggable: true, showConfirmButton: false, timer: 2200, timerProgressBar: true }); btnGuardar.disabled = false; btnGuardar.innerHTML = textoOriginal; });
    }
    ...
    function descargarConstancia(idEvaluacion) {
        ...
        const urlPDF = `/coordinador/union-hidalgo/complementarias/constancia/${idEvaluacion}/pdf?id_documento=${documentoMembrete}`;
        window.location.href = urlPDF;
    }
...
@include('coordinador.partials.constancia_complementarias_cargos_editable', ['constanciaCargosStorageKey' => 'constancia_cargos_complementarias_union'])
