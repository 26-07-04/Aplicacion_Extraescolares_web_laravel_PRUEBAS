...
<script>
    ...
    @php
        ...
        // Debug: ver qué semestre se está obteniendo
        if ($semestreActivo) {
            \Log::info('Semestre activo encontrado:', [
                'id' => $semestreActivo->id_semestre,
                'nombre' => $semestreActivo->nombre,
                'fecha_inicio' => $semestreActivo->fecha_inicio,
                'fecha_fin' => $semestreActivo->fecha_fin,
                'estatus' => $semestreActivo->estatus
            ]);
        } else {
            \Log::warning('No se encontró semestre activo');
        }
    ...
    // Debug: Ver qué datos del semestre se están cargando
    console.log('Semestre activo:', semestreActivo);
    ...
    function abrirModalEvaluacion(estudiante) {
            ...
            <div style="background: white; border-radius: 12px; max-width: 1000px; width: 100%; max-height: 90vh; overflow-y: auto; box-shadow: 0 10px 40px rgba(0,0,0,0.3);">
                ...
                <div style="padding: 30px;">
                    <form id="formEvaluacion">
                        ...
                        <!-- Sección 3: Datos para la Constancia -->
        ...
        // Cargar fechas del semestre activo
        ...
        criterios.forEach((criterio, index) => {
            ...
            html += `
                <tr style="background: ${index % 2 === 0 ? '#ffffff' : '#f8f9fa'};">
                    ...
                    <td style="padding: 12px; text-align: center; border: 1px solid #dee2e6; vertical-align: middle;">
                        <input type="radio" name="criterio${num}" value="4" class="criterio-radio" required style="width: 20px; height: 20px; cursor: pointer;">
                    </td>
                </tr>
            `;
        });
    ...
    function calcularResultados() {
        ...
        if (criteriosEvaluados === 7) {
            ...
            // Determinar nivel de desempeño según la Tabla 2
            let nivelDesempeno = '';
    ...
    function guardarEvaluacion(estudiante) {
        ...
        for (let i = 1; i <= 7; i++) {
            ...
            if (radioSeleccionado) {
                criterios.push({
                    numero: i,
                    valor: parseInt(radioSeleccionado.value)
                });
            }
        }
        ...
        // Validar campos obligatorios de profesor y jefe
        ...
        if (!jefeExtraescolares) {
            alert('Por favor, ingrese el nombre del Jefe del Departamento de Actividades Complementarias.');
        ...
        fetch('{{ route("demetrio.evaluacion.guardar") }}', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                ...
                const urlPDF = `/coordinador/demetrio-vallejo/complementarias/constancia/${data.id_evaluacion}/pdf?id_documento=${documentoMembrete}`;
                ...
                // Mostrar alerta de éxito (Swal) y auto cerrar
                setTimeout(() => {
                    Swal.fire({
    ...
    // Función para descargar constancia existente
    function descargarConstancia(idEvaluacion) {
        ...
        const urlPDF = `/coordinador/demetrio-vallejo/complementarias/constancia/${idEvaluacion}/pdf?id_documento=${documentoMembrete}`;
        window.location.href = urlPDF;
    }
