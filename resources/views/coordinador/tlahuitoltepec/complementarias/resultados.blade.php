...
@php
    // Reutilizamos la lógica de paginación y presentación en el JS
...
<div class="resultados-container">
    <div class="resultados-header">
        <div class="resultados-header-content">
            ...
            <p>
                <i class="fas fa-building"></i> {{ $unidad ?? 'Unidad Académica Santa María Tlahuitoltepec' }}
                | 
...
<script>
    const allEvaluaciones = {!! json_encode(($evaluaciones ?? collect())->values()->toArray()) !!};
    ...
    function renderizarPagina(pagina, filtro = '') {
        ...
        if (filtro) {
            evaluacionesFiltradas = allEvaluaciones.filter(ev => {
                ...
                return nombre.includes(filtro) || control.includes(filtro) || carrera.includes(filtro) || actividad.includes(filtro) || resultado.includes(filtro);
            });
        }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (btnAnterior) { btnAnterior.addEventListener('click', () => { if (paginaActual > 1) { renderizarPagina(paginaActual - 1, filtroActual); } }); }
        if (btnSiguiente) { btnSiguiente.addEventListener('click', () => { const totalPaginasFiltradas = Math.ceil((filtroActual ? allEvaluaciones.filter(ev => { const nombre = (ev.estudiante?.nombre || '').toLowerCase(); const control = (ev.estudiante?.numero_control || '').toLowerCase(); const carrera = (ev.estudiante?.carrera || '').toLowerCase(); return nombre.includes(filtroActual) || control.includes(filtroActual) || carrera.includes(filtroActual); }).length : allEvaluaciones.length) / evaluacionesPorPagina); if (paginaActual < totalPaginasFiltradas) { renderizarPagina(paginaActual + 1, filtroActual); } }); }
        renderizarPagina(1);
    });
    ...
    function abrirImpresion(tipo) {
        ...
        const baseUrl = `{{ route('coordinador.tlahuitoltepec.resultados.print.complementarias', ['id' => '__ID__']) }}`.replace('__ID__', semestreId);
        const url = tipo ? `${baseUrl}?tipo=${encodeURIComponent(tipo)}` : baseUrl;
        const iframe = document.createElement('iframe');
        iframe.style.position = 'fixed'; iframe.style.right = '0'; iframe.style.bottom = '0'; iframe.style.width = '0'; iframe.style.height = '0'; iframe.style.border = '0'; iframe.src = url; document.body.appendChild(iframe);
        iframe.onload = function() { try { const iframeWindow = iframe.contentWindow || iframe; if (iframeWindow && iframeWindow.print) { iframeWindow.focus(); iframeWindow.print(); } } catch (e) { console.error('Error al imprimir desde iframe:', e); } finally { setTimeout(() => { document.body.removeChild(iframe); }, 1000); } };
    }
</script>