...
<script>
    ...
    @php
        ...
        $semestreIdForQuery = $currentSemestre->id_semestre ?? $currentSemestre->id ?? null;
        foreach ($allEstudiantes as $est) {
            $status = 'pending';
            $idEvaluacion = null;
            if ($semestreIdForQuery) {
                $eval = \App\Models\Evaluacion::where('id_alumno', $est->id_alumno)
                    ->where(function($q) use ($est) {
                        // según esquema local la evaluación puede referenciar id_actividad o no; intentar por id_actividad
                        if (isset($est->id_actividad)) {
                            $q->where('id_actividad', $est->id_actividad);
                        }
                    })
                    ->where('id_semestre', $semestreIdForQuery)
                    ->first();
                if ($eval) {
                    $status = 'completed';
                    $idEvaluacion = $eval->id_evaluacion ?? $eval->id ?? null;
                }
            }
            $estudiantesArray[] = [
    ...
    function renderizarPagina(pagina) {
        ...
        if (estudiantesEnPagina.length === 0) {
            ...
            estudiantesEnPagina.forEach(s => {
                ...
                // Botón de acción: si ya fue evaluado, mostrar 'Descargar' y usar id_evaluacion; si no, mostrar 'Evaluar'
                let accionHtml = '';
                ...
                tr.innerHTML = `
                    ...
                    <td style="padding:12px 8px; border-bottom:1px solid #f2f2f2; text-align:center;">${accionHtml}</td>
                `;
    ...
    (function persistenciaDocumentoMembrete(){
        ...
        const semestreId = '{{ $semIdForStorage }}';
        ...
        modal.innerHTML = `
            <div style="background: white; border-radius: 12px; max-width: 1000px; width: 100%; max-height: 90vh; overflow-y: auto; box-shadow: 0 10px 40px rgba(0,0,0,0.3);">
                <div style="background: linear-gradient(135deg, #1a3461 0%, #2d5aa0 100%); color: white; padding: 25px 30px; border-radius: 12px 12px 0 0; position: relative;">
                    ...
                    <button onclick="cerrarModalEvaluacion()" style="position: absolute; top: 20px; right: 20px; background: rgba(255,255,255,0.2); border: none; color: white; width: 36px; height: 36px; border-radius: 50%; cursor: pointer; font-size: 1.3em;">
    ...
    function guardarEvaluacion(estudiante) {
        ...
        .then(data => {
            if (data.success) {
                ...
                const urlPDF = `/coordinador/tlahuitoltepec/complementarias/constancia/${data.id_evaluacion}/pdf?id_documento=${documentoMembrete}`;
    ...
    function descargarConstancia(idEvaluacion) {
        ...
        const urlPDF = `/coordinador/tlahuitoltepec/complementarias/constancia/${idEvaluacion}/pdf?id_documento=${documentoMembrete}`;
        window.location.href = urlPDF;
    }
...
@include('coordinador.partials.constancia_complementarias_cargos_editable', ['constanciaCargosStorageKey' => 'constancia_cargos_complementarias_tlahui'])
