...
<div class="seccion-contenedor">
  ...
  <div id="informesGeneradosContenedor" class="contenedor-informes-generados" style="background: #fff; border-radius: 12px; box-shadow: 0 4px 16px rgba(0,0,0,0.08); padding: 32px; margin: 40px 0; border-left: 6px solid #1a365d; display: block;">
          ...
          <div class="documento-card tarjeta-documento" style="background: #fff; border-radius: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.07); padding: 16px; min-width: 260px; max-width: 300px; display: flex; flex-direction: column; align-items: flex-start; border: 1px solid #e1e5eb;">
            <div class="documento-main" style="display: flex; align-items: center; width: 100%;">
              ...
              <div style="flex:1;">
                ...
                <div class="documento-info" style="font-size: 0.85rem; color: #888;">Unidad: {{ $inf->unidad->nombre ?? 'Sin unidad' }}</div>
              </div>
            </div>
            <div class="documento-actions" style="margin-top: 10px; display: flex; gap: 8px;">
    ...
    @else
      <div style="color:#888;">No hay informes generados para este semestre.</div>
    @endif
      ...
      <table class="tabla-eventos" id="tablaEventos">
        ...
        <tbody id="cuerpoTabla">
          @forelse($actividades as $i => $actividad)
            <tr>
              <td>
                <input type="text" value="{{ $i + 1 }}" disabled class="tabla-input">
              </td>
              <!-- nombre_actividad + abreviatura unidad -->
              <td>
                <input type="text"
                       class="tabla-input"
                       value="{{
                         $actividad->nombre_actividad . (
                           isset($actividad->unidad->nombre_unidad)
                             ? (
                                 (function($nombre, $id) {
                                   $nombre = strtolower(trim($nombre));
                                   if($id == 1) return ' - U.H.';
                                   if(strpos($nombre, 'union hidalgo') !== false) return ' - U.H.';
                                   if(strpos($nombre, 'demetrio vallejo') !== false) return ' - D.V.M.';
                                   if(strpos($nombre, 'tlahuitoltepec') !== false) return ' - S.M.T.';
                                   if(strpos($nombre, 'valle de etla') !== false) return ' - V.E.';
                                   return '';
                                 })($actividad->unidad->nombre_unidad, $actividad->unidad->id_unidad)
                               )
                             : ''
                         )
                       }}"
              ...
              <!-- institución organizadora editable -->
              <td>
                <input type="text" class="tabla-input" value="" placeholder="Institución organizadora">
              </td>
              <td><input type="date" class="tabla-input"></td>
              <td><input type="number" class="tabla-input" min="0" value="{{ $actividad->total_estudiantes ?? 0 }}" readonly></td>
              <td><input type="number" class="tabla-input input-small" min="0"></td>
              <td><input type="number" class="tabla-input input-small" min="0"></td>
              <td><input type="text" class="tabla-input"></td>
            </tr>
          @empty
            <tr>
