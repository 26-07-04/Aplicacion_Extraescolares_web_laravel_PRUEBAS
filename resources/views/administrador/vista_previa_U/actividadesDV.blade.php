...
<head>
  ...
  <title>Actividades Extraescolares - Demetrio Vallejo</title>
  ...
  <!-- Configuración única (como en UH) -->
  <script>
    (function(){
      ...
      function obtenerIdSemestre() {
        ...
        return qSemestre || serverSemestre || 2; // 2 como default para DV
      }

      // CAMBIO 2: Configuración única (igual que UH pero con unidadId diferente)
      window.ActDV = {
        rutas: {
          ...
          detail: "{{ url('administrador/vista_previa_U/D_actividades_DV') }}",
        },
        unidadId: 2,  // Demetrio Vallejo = 2
        semestreId: obtenerIdSemestre(),
        assetBase: "{{ asset('') }}"
      };
      
      console.log('Configuración ActDV:', window.ActDV);
    })();
  </script>
</head>
<body>
  <!-- CAMBIO 3: Agregar info del semestre como en UH -->
  <div class="wrapper">
    <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 15px;">
      <a href="{{ route('admin.principal', ['id' => request()->input('id_semestre', 2)]) }}"
    ...
    <main>
      ...
      <div class="unidad">
        Unidad Académica: Demetrio Vallejo Martínez en el Espinal
      </div>
      ...
      <div id="modal-actividad" class="modal">
        <div class="modal-contenido">
          ...
          <input type="hidden" id="id-semestre-actividad" value="{{ request()->input('id_semestre', 2) }}">
  ...
  <!-- IMPORTANTE: ELIMINA ESTA LÍNEA DUPLICADA DEL SCRIPT DE FONTAWESOME -->
  <!-- <script src="https://kit.fontawesome.com/your-fontawesome-kit.js" crossorigin="anonymous"></script> -->

  <!-- SCRIPT ÚNICO: actividades.js (el mismo que usa UH) -->
  <script src="{{ asset('js/actividades.js') }}"></script>
  
  <!-- CAMBIO 6: Script para manejar el semestre (igual que en UH) -->
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      // Actualizar display del semestre ID
      const semestreId = window.ActDV.semestreId;
      document.querySelectorAll('#current-semestre-id, #display-semestre-id').forEach(el => {
        el.textContent = semestreId;
      });
      
      console.log('Actividades DV cargadas para Semestre ID:', semestreId, 'Unidad ID:', window.ActDV.unidadId);
